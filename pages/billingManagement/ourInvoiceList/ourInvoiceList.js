...
Page({
	...
	getScreenList() {
		...
		getUserList("/billingManagement/ourInvoiceList/ourInvoiceList");
		getGroupList("/billingManagement/ourInvoiceList/ourInvoiceList");
		getClientList("/billingManagement/ourInvoiceList/ourInvoiceList");
		getSomeDateList("/billingManagement/ourInvoiceList/ourInvoiceList");
	...
	confirmData(e) {
		...
		if (type === "date") {
			...
			this.setData({
				dateName: e.detail.value[0].text === "全部" ? "" : e.detail.value[0].text,
			});
		}

		if (type === "order_or_other") {
			console.log(e.detail.value)
			this.data.typeName = e.detail.value[0].text;
			this.data.order_or_other = e.detail.value[0].id;
			this.setData({
				typeName: e.detail.value[0].text === "全部" ? "" : e.detail.value[0].text,
			});
		}
	...
	getList() {
		...
		wxReq({
			...
			"/billingManagement/ourInvoiceList/ourInvoiceList"
		).then((res) => {
