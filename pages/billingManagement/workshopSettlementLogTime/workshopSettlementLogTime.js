// pages/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime.js
const {
	getBillingList,
	wxReq,
	debounce,
	getUserList,
	getStaffList,
	getGroupList,
	getProcessList,
	getSomeDateList,
} = require("../../../utils/util");

Page({
	/**
	 * 页面的初始数据
	 */
	data: {
		showPopup: false,
		showLoading: false,
		noData: false,
		isEnd: false,
		userList: [],
		groupList: [],
		dateList: [],
		statusList: [],
		orderType: [],
		timeTypeList: [{
			text: '全部',
			id: ''
		}, {
			text: '小时',
			id: 1
		}, {
			text: '天',
			id: 2
		}],
		list: [],
		time_type: "",
		process_name: "",
		user_id: "",
		group_id: "",
		staff_id: "",
		is_check: "",
		order_type: "",
		start_time: "",
		end_time: "",
		page: 1,
	},

	onLoad(options) {
		this.getScreenList();
		const titles = [
			"创建人",
			"负责小组",
			"计时方式",
			"审核状态",
			"筛选员工",
			"创建时间",
		];
		const vtabs = titles.map((item) => ({
			title: item
		}));
		this.setData({
			list: [],
			vtabs
		});
		this.confirmData();
	},

	/**
	 * 生命周期函数--监听页面加载
	 */
	onShow(options) {
		if (wx.getStorageSync("isDo")) {
			this.getScreenList();
			this.setData({
				list: []
			});
			this.confirmData();
			wx.setStorageSync("isDo", false);
		}
	},

	// 拿到筛选列表
	getScreenList() {
		let arrs = [{
			text: "全部",
			id: "",
			children: [{
				text: "全部",
				id: "",
			}, ],
		}, ];
		getUserList("/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime");
		getGroupList("/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime");
		getStaffList("/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime");
		getSomeDateList("/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime");
		getProcessList("/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime");

		this.setData({
			user_id: "",
			userList: wx.getStorageSync("userList").map((item) => {
				return {
					id: item.id,
					text: item.text,
					checked: false,
				};
			}),
			group_id: "",
			groupList: wx.getStorageSync("groupList").map((item) => {
				return {
					id: item.id,
					text: item.text,
					checked: false,
				};
			}),
			start_time: "",
			end_time: "",
			processList: arrs.concat(wx.getStorageSync("processList")),
			dateList: wx.getStorageSync("someDateList").map((item) => {
				return {
					id: item.id,
					text: item.text,
					checked: false,
				};
			}),
			staff_id: "",
			staffList: wx.getStorageSync("staffList").map((item) => {
				return {
					id: item.id,
					text: item.text,
					checked: false,
				};
			}),
			is_check: "",
			statusList: [{
					text: "全部",
					id: "",
					checked: false
				},
				{
					text: "待审核",
					id: 0,
					checked: false
				},
				{
					text: "已审核",
					id: 1,
					checked: false
				},
				{
					text: "已驳回",
					id: 2,
					checked: false
				},
			],
		});
	},

	// 打开选择器
	openPicker(e) {
		const {
			type
		} = e.currentTarget.dataset;
		if (type === "process") {
			this.setData({
				showProcess: true,
			});
		}
	},

	// 关闭选择器
	closeShowPicker(e) {
		const {
			type
		} = e.currentTarget.dataset;
		if (type === "process") {
			this.setData({
				showProcess: false,
			});
		}
	},

	// 选择器提交
	confirmData(e) {
		let type;
		if (e?.currentTarget) {
			type = e.currentTarget.dataset.type;
		}

		if (type === "process") {
			this.checkProcess(e.detail.value[1]);
		}

		this.data.page = 1;
		this.setData({
			list: [],
			isEnd: false,
			noData: false,
		});
		this.reqOrder();
		this.closeShowPicker(e);
		this.closePopup()
	},

	// 选择工序
	checkProcess(e) {
		const {
			text
		} = e;
		this.setData({
			process_name: text === '全部' ? '' : text
		});
	},

	// 打开选择框
	openPopup() {
		this.setData({
			showPopup: true,
		});
	},

	// 关闭选择框
	closePopup() {
		this.setData({
			showPopup: false,
		});
	},

	// 更改选择
	bindPickerChangeAge(e) {
		const {
			type
		} = e.currentTarget.dataset;
		let index = e.detail.value;

		if (type === "user") {
			this.setData({
				user_id: this.data.userList[index].id,
			});
		}

		if (type === "group") {
			this.setData({
				group_id: this.data.groupList[index].id,
			});
		}

		if (type === "staff") {
			this.setData({
				staff_id: this.data.staffList[index].id,
			});
		}

		if (type === "status") {
			this.setData({
				is_check: this.data.statusList[index].id,
			});
		}

		if (type === "order_type") {
			this.setData({
				order_type: this.data.orderType[index].id,
			});
		}
		
		if (type === "time_type") {
			this.setData({
				time_type: this.data.timeTypeList[index].id,
			});
		}

		if (type === "date") {
			this.setData({
				start_time: this.data.dateList[index].id[0],
				end_time: this.data.dateList[index].id[1],
			});
		}
	},

	// 节流
	reqOrder: debounce(function () {
		this.getList();
	}, 1000),

	// 点击确认
	getList() {
		if (this.data.isEnd) {
			return;
		}

		this.setData({
			showLoading: true,
		});

		let {
			is_check,
			user_id,
			group_id,
			staff_id,
			time_type,
			process_name,
			start_time,
			end_time,
			page,
		} = this.data;
		wxReq({
				url: "/production/inspection/lists",
				method: "GET",
				data: {
					is_check,
					user_id,
					type: 2,
					group_id,
					staff_id,
					time_type,
					process_name,
					start_time,
					end_time,
					page,
					limit: 10,
				},
			},
			"/billingManagement/workshopSettlementLogTime/workshopSettlementLogTime"
		).then((res) => {
			let data = res.data.data.items;

			if (data.length < 10) {
				this.setData({
					isEnd: true,
				});
			}

			if (this.data.page === 1 && data.length === 0) {
				this.setData({
					noData: true,
					showLoading: false,
				});
			}

			let list = this.data.list.concat(res.data.data.items);
			let additional = res.data.data.additional;
			additional.total_number = (additional.total_number / 10000).toFixed(2);

			additional.total_price = (additional.total_price / 10000).toFixed(2);

			this.data.page += 1;
			this.setData({
				showLoading: false,
				list,
				additional,
			});
		});
	},

	toIndex() {
		wx.reLaunch({
			url: "/pages/billingManagement/index",
		});
	},
});