...
// pages/workshopEditByThing/workshopEditByThing.js
Page({
  /**
   * 页面的初始数据
   */
  ...
  getDetail() {
    wxReq(
      {
        url: "/production/inspection/lists?ids[]=" + this.data.id + "&type=1",
        method: "GET",
      },
      "/workshopEditByThing/workshopEditByThing?id=" + this.data.id
    ).then((res) => {
  ...
  changeType(e) {
    ...
    info.total_price = (info.price || 0) * (info.number || 0);
